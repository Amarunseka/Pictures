...
class FilesViewController: UIViewController {
    ...
    private func sorting() {
        ...
        if sortingModel.isSorting {
            content.sort {
                let compareResult = $0.lastPathComponent.lowercased().compare($1.lastPathComponent.lowercased(), options: .numeric)
                if sortingModel.sortingMode == 0 {
...
// MARK: - CollectionView
extension FilesViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if type[.type] as! FileAttributeType == FileAttributeType.typeDirectory {
            let vc = FilesViewController(directoryURL: url)
